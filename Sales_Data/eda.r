...
ggplot(sales_data, aes(x = date, y = revenue)) +
    ...
    theme_minimal() # so all this is same as before, just dk what alpha is and why we do theme_minimal()?
...
ggplot(sales_data, aes(x = category, y = revenue, fill = category)) +
    ...
    labs(title = "Revenue Distribution by Category", x = "Category", y = "Revenue") +
    theme_minimal() +
...
ggsave("outputs/figures/monthly_revenue_trend.png", width = 8, height = 5) # this is the plot save funcn as photo. i should play with the width and the height more could be interesting.
